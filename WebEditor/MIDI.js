...
function afterLoad() {
    ...
    for (var c = 1; c < 5; c++) {
        ...
        var html = ['<div style = "padding: 5px;">Type</div>', '<div><select id="cType' + c + '" class="docNav" onchange="updateCV(' + c + ', cfgEnum.type);"></select></div>', '<div class="controllerControl" style = "padding: 5px;">Controller</div>', '<div class="controllerControl"><select id="cController' + c + '" class="docNav" onchange="updateCV(' + c + ', cfgEnum.controller);"></select></div>', '<div class="channelControl" style = "padding: 5px;">Channel</div>', '<div class="channelControl"><select id="cChannel' + c + '" class="docNav" onchange="updateCV(' + c + ', cfgEnum.channel);"></select></div>', '<div>', '	<button class="topcoat-button-bar__button--large" onclick="testOutput(controlEnum.cv, ' + c + ');">Test</button>', '</div>', '<div id="connectionStatus" style="color: #d6756a; text-align: right;">' + c + '</div>'].join("\n");
    ...
    for (var g = 1; g < 9; g++) {
        ...
        var html = ['<div style = "padding: 5px;">Type</div>', '<div><select id="gType' + g + '" class="docNav" onchange="updateGate(' + g + ', cfgEnum.type);"></select></div>', '<div class="gateControl" style = "padding: 5px;">Note</div>', '<div class="gateControl"><select id="gNote' + g + '" class="docNav" onchange="updateGate(' + g + ', cfgEnum.specificNote);"></select></div>', '<div class="channelControl" style = "padding: 5px;">Channel</div>', '<div class="channelControl"><select id="gChannel' + g + '" class="docNav" onchange="updateGate(' + g + ', cfgEnum.channel);"></select></div>', '<div>', '	<button class="topcoat-button-bar__button--large" onclick="testOutput(controlEnum.gate, ' + g + ');">Test</button>', '</div>', '<div id="connectionStatus" style="color: rgb(125, 206, 115); text-align: right;">' + g + '</div>'].join("\n");
        ...
        for (var i = 24; i < 97; i++) {
            var el = document.createElement("option");
            el.textContent = noteNames[i % 12] + (Math.floor(i / 12) - 1);
            // C1 (24) to C7 (96)
            el.value = i;
